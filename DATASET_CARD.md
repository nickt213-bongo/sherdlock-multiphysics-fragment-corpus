...
Public training labels contain fragment membership, distractor state, discrete poses, and the oracle brace set. Private test answers additionally contain true seam pairs, per-brace load effects, base load deficits, interference penalties, and normalization constants. Those private evaluator values are never copied to participant files.
